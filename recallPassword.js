/**
 * https://js.checkio.org/mission/cipher-map/
 */

var chiperMap = {
		'0,0': [0,3], '0,1': [1,3], '0,2': [2,3], '0,3': [3,3],
		'1,0': [0,2], '1,1': [1,2], '1,2': [2,2], '1,3': [3,2],
		'2,0': [0,1], '2,1': [1,1], '2,2': [2,1], '2,3': [3,1],
		'3,0': [0,0], '3,1': [1,0], '3,2': [2,0], '3,3': [3,0],
}

function recallPassword(grille, password) {
	var result = "";
	var position = [];
	for (var i=0; i<4; i++) {
		for (var j=0; j<4; j++) {
			if (grille[i][j]=='X') position.push(i.toString() + ',' + j.toString())
		}
	}
	for (var i=0; i<4; i++) {
		for (var j=0; j<4; j++) {
			var m = position[j].split(',').map(x=>parseInt(x))
			result += password[m[0]][m[1]]
			var n = chiperMap[position[j]]
			position[j] = n[0].toString() + ',' + n[1].toString()
		}
		position.sort()
		console.log(position)
	}
	console.log(result)
}

recallPassword(
	    ['X...',
	     '..X.',
	     'X..X',
	     '....'],
	    ['itdf',
	     'gdce',
	     'aton',
	     'qrdi']) == 'icantforgetiddqd'

	recallPassword(
	    ['....',
	     'X..X',
	     '.X..',
	     '...X'],
	    ['xhwc',
	     'rsqx',
	     'xqzz',
	     'fyzr']) == 'rxqrwsfzxqxzhczy'